	...
	<div class="col-6 col-md-4 d-flex flex-column">
		...
		<h5 class="text-secondary ">
			<a class="link-seguidos" data-toggle="modal"
				data-target="#seguidores" id="contadorSeguidores">
...
<section class="row mt-3">
	...
	<article class="col-12 col-md-4">
		<c:if test="${not empty usuarioLogeado }">
			...
			<c:if test="${not empty usuarioLogeado}">
				<input class="col-12 form-control" type="hidden" 
				value="${usuarioLogeado.getId()}" id="idUsuarioEnviaMensaje"/>
				<input class="col-12 form-control" id="idUsuarioRecibeMensaje"
					type="hidden" value="${usuarioPerfil.getId()}" />
				<input class="col-12 form-control" id="nombreUsuarioRecibeMensaje"
					type="hidden" value="${usuarioPerfil.getNombreUsuario()}" />
				<button type="submit" class="btn btn-naranja" 
					data-toggle="modal" data-target="#enviarMensaje" id="enviarMensajeAUsuarioBoton">Enviar mensaje</button>
			</c:if>
		</c:if>
		<c:if test="${empty usuarioLogeado}">
			<a href="login?errorSeguir=true" class="btn btn-naranja">Seguir</a>
		</c:if>
		<div id="resultadoEnviarMensaje">
		</div>
	</article>
</section>
<hr>
<section class="row">
	<article class="col-12 col-md-4">
		<h4 class="text-center text-secondary">Categorias seguidas por
			${usuarioPerfil.getNombreUsuario()}</h4>
		<div class="">
					...
					<div class="owl-carousel owl-theme">
						<c:forEach items="${categorias}" var="categoria">
							<c:if test="${categoria.getTipoCategoria() == 'JUEGOS'}">
								<div class="item">
									<a href="juegosOVarios?categoriaId=${categoria.getId()}"> <img
										class="owl-lazy" data-src="${categoria.getUrlImagen()}">
									</a>
								</div>
							</c:if>
						</c:forEach>
					...
					<div class="d-flex justify-content-center">
						<c:forEach items="${categorias}" var="categoria">
							<c:if test="${categoria.getTipoCategoria() == 'JUEGOS'}">
								...
								<a href="juegosOVarios?categoriaId=${categoria.getId()}"
									class="m-3"> <img class="img-fluid"
									src="${categoria.getUrlImagen()}">
								</a>
	...
	<article class="col-12 col-md-8">
					...
					<c:forEach items="${publicaciones}" var="publicacionDelFor">
						<div class="col-12 mt-5 mb-5 rounded-lg">
							...
							<c:if test="${publicacionDelFor.getEstado() == 'ACTIVO'}">
								...
								<div
									class="d-flex justify-content-between p-2 bg-fondo rounded-top">
									<div class="col-4">
										<h3>
											<a
												href="perfil?usuarioNombre=${publicacionDelFor.getUsuario().getNombreUsuario() }"
												class="nombreUsuarioPublicacion">
									...
									<div class="col-4 text-right ">
										<div class="d-flex justify-content-end">
											...
											<a
												href="juegosOVarios?categoriaId=${publicacionDelFor.getCategoria().getId()}">
                            ...
                            border-right-0 border-left-0 border-warning">
									<c:if test="${not empty usuarioLogeado}">
										<div class="d-flex justify-content-center">
											<button type="submit"
												class="btn btn-outline-naranja hoverIconsBotonIcono"
												value="${publicacionDelFor.getId()}"
												name="idPublicacionADarLike${publicacionDelFor.getId()}"
												id="idPublicacionADarLike${publicacionDelFor.getId()}">
												<i class="far fa-thumbs-up"></i>
											</button>
											<p class="text-secondary"
												id="cantidadLikesPublicacion${publicacionDelFor.getId()}">
												${publicacionDelFor.getCantidadLikes()}</p>
										</div>
									</c:if>
									<c:if test="${empty usuarioLogeado}">
										<div class="d-flex justify-content-center">
											<a href="login?errorLike=true"
												class="btn btn-outline-naranja hoverIconsBotonIcono"> <i
									...
									<div class="d-flex justify-content-center">
										<a type="button"
											class="btn btn-outline-naranja botonCollapseComentarios hoverIconsBotonIcono"
											data-id="${publicacionDelFor.getId()}" data-toggle="collapse"
											href="#collapseComentarios"><i
											class="far fa-comment-dots"></i></a>
		...
		<div>
			...
			<div class="modal fade" id="enviarMensaje" tabindex="-1"
				aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content bg-fondo ">
						...
						<div class="modal-headerp-3 pt-3">
							<form action="crearMensaje"class="container"
								method="POST" id="mandarMensajeUsuarioFormulario">
								<div>
									<textarea id="mensajeAMandarAUsuario"
										name="mensajeAMandarAUsuario" class="form-control " rows="3"></textarea>
								</div>
								<div class="modal-footer">
									<input class="col-12 form-control" type="hidden" 
									name="idUsuarioEnviaMensaje" id="idUsuarioEnviaMensaje"/>
									<input class="col-12 form-control" id="idUsuarioRecibeMensaje" 
									type="hidden" name="idUsuarioRecibeMensaje" />
